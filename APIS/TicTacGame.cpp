...
class GameRenderer {
public:
	GameRenderer (const int rowSize , const int colSize){
	...
	int GetCurrentBoxIdx () const { return this->currentBoxIdx;}
	
   	bool IsEmptyBox (int idx){ return arr[idx] != PLAYER1_BOX && arr[idx] != PLAYER2_BOX; }
	...
	public:
		...
		void ListenToKeys (){
			while(true){
				...
				_CH_
				// @firing Events
				LISTEN_KEY (VK_UP    , [&] () { OnUpBtnPressed.Invoke();   });
				LISTEN_KEY (VK_DOWN  , [&] () { OnDownBtnPressed.Invoke(); });
				LISTEN_KEY (VK_LEFT  , [&] () { OnLeftBtnPressed.Invoke(); });
				LISTEN_KEY (VK_RIGHT , [&] () { OnRightBtnPressed.Invoke();});
				
				LISTEN_KEY (VK_RETURN ,[&]()  { OnEnterBtnPressed.Invoke();});
		...
		*/
		...
		void SetTick (bool state){ this->tick = state;} // if false this will not listen for inputs anymore
		
	private:	
	  InputManager(){ tick = true;}	
	  static InputManager* instance;
	  ~InputManager(){ delete instance;}
	  bool tick;
	  template<class CallBack>
	  void LISTEN_KEY (int key ,CallBack callBack ){ if(GetAsyncKeyState(key) < 0 ) callBack(); }
