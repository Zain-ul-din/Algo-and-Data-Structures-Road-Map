...
namespace FileSystem {
    ...
    static void CreateFile (string& fileName , bool showLog = true) {
        if(!IsExists(fileName)) return;
        if(showLog && !isLogged) Input::Warning(stringstream( "\n FM LOG : New File ` " + fileName + " ` Has Been Created. ! In case file storage is not working. Please check you're window defender may be WD is conflicting with fileSystem\n\n").str().c_str());
        isLogged = true;
        ofstream out;
