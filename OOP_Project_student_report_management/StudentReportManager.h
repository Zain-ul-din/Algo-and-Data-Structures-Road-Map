...
public:
    ...
    void Init () {
         ...
         if (!FileSystem::IsExists(_fileName))
         ...
         if(!FileSystem::IsExists(this->_secFileName))
            FileSystem::CreateFile(this->_secFileName);
        ...
        if (!any_of(this->filesRecord.begin() , this->filesRecord.end() , [&](string& str){
         ...
         if (!FileSystem::IsExists(fileName)) {
            ...
            for (int i = 0 ; i < student.getSubjectMarks().size() ; i ++)
                Input::PrintStr(
                   Utilities::DoubleToString(student.getSubjectMarks()[i]).c_str(), 20 ,'|',
                      Utilities::GetGradeColor(this->subjects[i].getTotalMarks() , student.getSubjectMarks()[i])
                );
            Input::Print("\n");
        }
    }
...
protected:
    ...
    void AddStudent () {
       ...
       for (int i = 0 ; i < subjects.size() ; i += 1) {
           cout << "Enter " << subjects[i].getName() << " Marks / " << subjects[i].getTotalMarks() << " _ ";
    ...
    void UpdateIndividual () {
        this->PrintResult(); Input::Print("\n");
        int rollNo = abs(Input::GetInput<int>(" >> Enter Roll No of Student _ "));
        Student student; student.setRollNo(rollNo);
    ...
    void UpdateResultCard () {
        ...
        PrintResult_Helper_Data({*val}); Input::Print("\n");
        ...
        double marks = abs(Input::GetInput<double>(stringstream ("Enter New Marks for" + this->subjects[idx].getName() + " / " +
    ...
    void UpdateForEachResult_Helper_Section () {
        ...
        if (!CheckValidIdx(this->subjects , idx , "\n \t** All Subjects Record ** \n"))
            return;
        double marks = abs(Input::GetInput<double>(stringstream (" >> Enter New Marks for All Students " + this->subjects[idx].getName() + " / " +
                                                                 to_string(this->subjects[idx].getTotalMarks()) + " ").str().c_str()));
    ...
    void UpdateForEachResult_Helper_UpdateAll () {
        ...
        if (!CheckValidIdx(this->subjects , idx , "\n \t** All Subjects Record ** \n"))
            return;
        double marks = abs(Input::GetInput<double>(stringstream (" >> Enter New Marks for All Sections " + this->subjects[idx].getName() + " / " +
                                                                 to_string(this->subjects[idx].getTotalMarks()) + " ").str().c_str()));
        if (marks > this->subjects[idx].getTotalMarks())
            marks = this->subjects[idx].getTotalMarks();
        if(!Input::GetChoice(" >> Apply For All Y/N : "))
    ...
    void FillResultCards_Helper (int& idx , Student& student , bool& isContainsNull ) {
        ...
        do {
            ...
            cout << "\n >> Enter Marks of " << this->subjects[idx].getName() << " / " << totalMarks << " _ ";
            double marks = abs(Input::GetInput<double>(" "));
            ...
            student.setPercentage(Utilities::GetPercentage(this->subjectsTotalMarks , totalSubjectsMarks));
            student.setTotalSubjects(this->subjects.size());
